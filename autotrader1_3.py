...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            spread = (ask_prices[0] - bid_prices[0]) * 3  # Use a wider spread for more liquidity
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position + LOT_SIZE < POSITION_LIMIT:
                ...
                self.send_insert_order(
                    self.bid_id, Side.BUY, new_bid_price, LOT_SIZE, Lifespan.G)
            ...
            if self.ask_id == 0 and new_ask_price != 0 and self.position - LOT_SIZE > -POSITION_LIMIT:
                ...
                self.send_insert_order(
                    self.ask_id, Side.SELL, new_ask_price, LOT_SIZE, Lifespan.G)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            self.send_hedge_order(next(self.order_ids), Side.ASK,
                                  MIN_BID_NEAREST_TICK + TICK_SIZE_IN_CENTS*3, volume)
        elif client_order_id in self.asks:
            self.position -= volume
            self.send_hedge_order(next(self.order_ids), Side.BID,
                                  MAX_ASK_NEAREST_TICK - TICK_SIZE_IN_CENTS*3, volume)
