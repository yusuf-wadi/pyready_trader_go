...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            new_bid_price = bid_prices[0] - int(TICK_SIZE_IN_CENTS * int(math.log(dynamic_aggressiveness**3)+1))
            new_ask_price = ask_prices[0] + int(TICK_SIZE_IN_CENTS * int(math.log(dynamic_aggressiveness**3)+1))
            #print(dynamic_aggressiveness)
            min_lot_size = 20
            max_lot_size = 60
            adjusted_lot_size = min_lot_size + int((max_lot_size - min_lot_size) * self.normalized_atr)

            self.movingAV = self.calc_rolling_av()
            self.movingSD = self.calc_rolling_sd()
            if self.movingSD != 0:
                Z = (self.mid_price - self.movingAV) / self.movingSD
            else:
                Z = 1
            ...
            # Set the threshold for volatility
            # volatility_threshold = 0.8

           
            # # Adjust HEDGE_RATIO based on normalized ATR
            # dynamic_hedge_ratio = HEDGE_RATIO_MIN + (HEDGE_RATIO_MAX - HEDGE_RATIO_MIN) * self.normalized_atr
            # hedge_volume = int(adjusted_lot_size * dynamic_hedge_ratio)
            hedge_interval = min(int((30/abs(dynamic_aggressiveness-0.232056552)**2.3)),50)  # in seconds
            ...
            if current_time - self.last_hedge_time > hedge_interval:
                if(pos_dif) > 0:
                    ...
                    hedge_price = MAX_ASK_NEAREST_TICK - hedge_p_scale
                    self.send_hedge_order(next(self.order_ids), Side.BID, MAX_ASK_NEAREST_TICK ,abs(pos_dif))
                    self.last_hedge_time = time.time()
                if(pos_dif) < 0:
                    self.futPostion -= abs(pos_dif)
                    hedge_price = MIN_BID_NEAREST_TICK + hedge_p_scale
