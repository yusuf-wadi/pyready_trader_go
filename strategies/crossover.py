...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            self.midETF = (bid_prices[0] + ask_prices[0]) / 2
            if self.midFutures == 0:
                return
        if instrument == Instrument.FUTURE:
            self.midFutures = (bid_prices[0] + ask_prices[0]) / 2
            if self.midETF == 0:
                return
            
        self.currentRatio = self.midETF / self.midFutures
        ...
        if self.tick >= WINDOW_SIZE:
            ...
            self.movingAverage = self.calculate_rolling_average()
            self.movingSD = self.calculate_rolling_sd()
            
            self.sma = self.calculate_sma(self.sma_window)

            # Calculate Z-score using bollinger bands and SMA crossover
            bollinger_upper = self.movingAverage + self.sd_multiplier * self.movingSD
            bollinger_lower = self.movingAverage - self.sd_multiplier * self.movingSD
            sma_crossover = self.movingAverage - self.sma

            #price_adjustment = - (self.position // LOT_SIZE) * TICK_SIZE_IN_CENTS
            new_bid_price = bid_prices[0] if bid_prices[0] != 0 else 0
            new_ask_price = ask_prices[0] if ask_prices[0] != 0 else 0

            # Cancel any existing orders that are no longer valid
            ...
            if sma_crossover > 0 and self.currentRatio > bollinger_upper: # sell signal
            ...
            elif sma_crossover < 0 and self.currentRatio < bollinger_lower: # buy signal
    ...
    def calculate_sma(self, window_size: int) -> float:
        """Calculate the simple moving average of the rolling prices
        """
        return sum(self.rollingPrices[-window_size:]) / window_size