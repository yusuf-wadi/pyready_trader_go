...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            self.currETF = (bid_prices[0] + ask_prices[0]) / 2
            if self.currFUT == 0:
                return
        if instrument == Instrument.FUTURE:
            self.currFUT = (bid_prices[0] + ask_prices[0]) / 2
            if self.currETF == 0:
                return

        self.currentRatio = self.currETF / self.currFUT
        self.rollingRatios.append(self.currentRatio)
    ...
    def calculate_rolling_average_50(self):
        ...
        for i in range(self.tick -WINDOW_SIZE, self.tick-1):
            sum += self.rollingRatios[i]
        return sum /WINDOW_SIZE

    def calculate_rolling_sd_50(self):
        sum = 0
        for i in range(self.tick -WINDOW_SIZE, self.tick-1):
            sum += (self.rollingRatios[i] - self.movingAverage) ** 2
    ...
    # Calculate the rsi
        rsi_window = 20 
        if len(self.rollingRatios) > rsi_window:
            diff = np.diff(self.rollingRatios[-rsi_window:])
