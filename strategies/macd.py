...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if self.tick >=WINDOW_SIZE:
             # Calculate MACD
            self.calculate_ema_23()
            self.calculate_ema_50()
            macd = self.ema23 - self.ema50
            ...
            buy_cond1 = (histogram > 0 and self.stoch_rsi > 0.6)
            buy_cond2 = (Z < 0.9)
            sell_cond1 = (histogram < 0 and self.stoch_rsi < 0.44)
            sell_cond2 = (Z > 1.1)
            
            print(f'HISTO: {histogram}\nStochRSI: {self.stoch_rsi}\nZ-score: {Z}')
            if sell_cond1:
                
                # if (Z > 1.1 and self.stoch_rsi > 0.63):
                #     print("\nsell cond 2\n")
                # else:
                #     print("\nsell cond 1\n")
                    
                # sell and stay in pos
                ...
                # if (Z < 0.89 and self.stoch_rsi < 0.37):
                #     print("\nbuy cond 2\n")
                # else:
                #     print("\nsell cond 1\n")
    ...
    def calculate_ema_23(self) -> float:
        low_ema = 23
        if len(self.rollingPrices) < low_ema:
            return sum(self.rollingPrices) / len(self.rollingPrices)
        else:
            alpha = 2 / (low_ema + 1)
            self.ema23 = self.rollingPrices[-1]
            for price in reversed(self.rollingPrices[-low_ema:-1]):
                self.ema23 = alpha * price + (1 - alpha) * self.ema23
                
    def calculate_ema_50(self) -> float:
        high_ema = 50
        if len(self.rollingPrices) < high_ema:
            return sum(self.rollingPrices) / len(self.rollingPrices)
        else:
            alpha = 2 / (high_ema + 1)
            self.ema50 = self.rollingPrices[-1]
            for price in reversed(self.rollingPrices[-high_ema:-1]):
                self.ema50 = alpha * price + (1 - alpha) * self.ema50
                
    def calculate_signal(self) -> float:
        if len(self.rollingPrices) < 26:
            return sum(self.rollingPrices) / len(self.rollingPrices)
        else:
            sig_cons = 17
            self.calculate_ema_23()
            ...
            #9/26
            alpha = 2 / (sig_cons + 1)
            self.signal_line = self.ema23 - self.ema50
            for price in reversed(self.rollingPrices[-sig_cons:-1]):
                self.signal_line = alpha * price + (1 - alpha) * self.signal_line
